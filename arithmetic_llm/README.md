...
Run commands from the repository root and prefer the canonical form:
- `uv run python -m arithmetic_llm.<module>`
...
```bash
uv run python -m arithmetic_llm.run_evaluation --model-path models/foundational_YYYYMMDD_HHMMSS/best_model.pt --tokenizer-path data/tokenizer --max-gen-length 512 --num-samples 100 --batch-size 1
```

PowerShell multiline example:
```powershell
uv run python -m arithmetic_llm.run_evaluation `
...
uv run python -m arithmetic_llm.generate_foundational_plaintext \
  --num-samples 100000 \
  --max-depth 4 \
  --num-range 1 20 \
  --invalid-rate 0.05 \
...
uv run python -m arithmetic_llm.generate_instruction_corpus_mixed \
  --num-samples 20000 \
  --max-depth 4 \
  --num-range 1 20 \
  --invalid-rate 0 \
...
uv run python -m arithmetic_llm.generate_corpus \
  --instruction-only \
  --num-samples 1000 \
  --max-depth 4 \
...
uv run python -c "import sys; [print(f'{sum(1 for _ in open(f))} {f}') for f in ['data/foundational_corpus.txt', 'data/instruction_corpus.txt', 'data/instruction_corpus_test.txt']]"

# 2. Train tokenizer
uv run python -m arithmetic_llm.train_tokenizer \
  --corpus-path data/foundational_corpus.txt \
  --output-dir data/tokenizer \
  --vocab-size 1000

# show tokenizer table
uv run python -m arithmetic_llm.print_token_table --tokenizer_path data/tokenizer/tokenizer.pkl  > tokens.csv

# Analyze your instruction corpus
uv run python -m arithmetic_llm.check_sequence_lengths \
  --corpus-path data/instruction_corpus.txt \
  --tokenizer-path data/tokenizer

# 3. Train foundational model
uv run python -m arithmetic_llm.run_foundational_training \
  --corpus-path data/foundational_corpus.txt \
  --output-dir models/ \
  --tokenizer-path data/tokenizer \
  --num-epochs 10 \
...
uv run python -m arithmetic_llm.run_instruction_training \
...
uv run python -m arithmetic_llm.run_instruction_training_lora \
  --instruction-corpus-path data/instruction_corpus.txt \
  --output-dir models/ \
  --tokenizer-path data/tokenizer \
  --foundational-checkpoint models/foundational_YYYYMMDD_HHMMSS/best_model.pt \
...
uv run python -m arithmetic_llm.run_grpo_training \
...
- `uv run python -m arithmetic_llm.generate_instruction_corpus_mixed` creates a mixed instruction corpus
- `--output-mixed`: Path for the mixed instruction corpus

**Foundational Plain-Text Script:**
- `uv run python -m arithmetic_llm.generate_foundational_plaintext` generates shuffled plain text directly
...
```bash
uv run python -m arithmetic_llm.merge_lora_adapter \
...
```bash
uv run python -m arithmetic_llm.run_interactive \
  --model-path models/instruction_YYYYMMDD_HHMMSS/best_model.pt \
  --tokenizer-path data/tokenizer
```
...
- Entry points: `arithmetic_llm/run_foundational_training.py`, `arithmetic_llm/run_instruction_training.py`, `arithmetic_llm/run_instruction_training_lora.py`, `arithmetic_llm/run_grpo_training.py`.
- Training loops: `arithmetic_llm/train_foundational.py`, `arithmetic_llm/train_instruction.py`, `arithmetic_llm/train_instruction_lora.py`, `arithmetic_llm/grpo_trainer.py`.
- Sampler/dataloader wiring: `arithmetic_llm/data_loader.py` (use distributed samplers and rank-aware shuffling).
- Checkpointing and resume: `arithmetic_llm/train_foundational.py` (`save_checkpoint` / `load_checkpoint`), `arithmetic_llm/grpo_trainer.py` (`save_checkpoint` / `load_checkpoint`), plus resume handling in instruction training scripts.
