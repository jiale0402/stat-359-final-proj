...
class GRPOTrainer:
    ...
    def __init__(
        ...
        self.optimizer = None
        self.scheduler = None
        self.verifier = verifier if verifier is not None else ArithmeticVerifier()
    ...
    def _score_candidate(
        ...
        if hasattr(self.verifier, "score_response"):
            score = self.verifier.score_response(generated_text, ground_truth)
        else:
            score = {"reward": self.verifier.compute_reward(generated_text, ground_truth)}

        if "reward" not in score:
            raise ValueError("verifier score_response must return a 'reward' value")
        return {key: float(value) for key, value in score.items()}

    def _build_response_token_mask(
        self,
        attention_mask: torch.Tensor,
        prompt_lens: List[int]
    ) -> torch.Tensor:
        """Mask next-token positions so only response tokens are included."""
        valid_mask = attention_mask[:, 1:] > 0
        seq_len = valid_mask.shape[1]
        positions = torch.arange(seq_len, device=attention_mask.device)
        prompt_lens_tensor = torch.as_tensor(
            prompt_lens,
            device=attention_mask.device,
            dtype=torch.long,
        )
        start_indices = torch.clamp(prompt_lens_tensor - 1, min=0)
        start_mask = positions.unsqueeze(0) >= start_indices.unsqueeze(1)
        return valid_mask & start_mask

    def _optimizer_step(self) -> None:
        if self.optimizer is None or self.policy_model is None:
            raise ValueError("optimizer and policy_model must be initialized")
    ...
    def _log_wandb(self, payload: Dict[str, float], step: int) -> None:
    ...
    def compute_kl_divergence(
        ...
        mask = token_mask.to(device=kl_sum.device, dtype=kl_sum.dtype)
        if kl_sum.shape != mask.shape:
            raise ValueError(
                "token_mask must match next-token KL shape, got "
                f"{mask.shape} and {kl_sum.shape}"
            )
        denom = torch.clamp(mask.sum(), min=1.0)
        return torch.sum(kl_sum * mask) / denom
    ...
    def train_step(
        ...
        for prompt_idx, prompt in enumerate(prompts):
            ...
            for cand_idx in range(num_candidates):
                ...
                score = self._score_candidate(text, ground_truth[prompt_idx])
                rewards_list.append(score["reward"])
                for key, value in score.items():
                    component_lists.setdefault(key, []).append(value)
        ...
        for start in range(0, total_candidates, sub_batch):
            ...
            token_mask = self._build_response_token_mask(
                attention_mask=attention_mask,
                prompt_lens=chunk_prompt_lens,
            )
            log_prob_sum = (token_log_probs * token_mask).sum(dim=1)

            policy_log_probs = torch.log_softmax(policy_logits[:, :-1, :], dim=-1)
            reference_log_probs = torch.log_softmax(
                reference_logits[:, :-1, :], dim=-1
            )
            policy_probs = torch.softmax(policy_logits[:, :-1, :], dim=-1)
            kl = policy_probs * (policy_log_probs - reference_log_probs)
            token_kl = torch.sum(kl, dim=-1)
            kl_mask = token_mask.to(dtype=token_kl.dtype)
            kl_denom = torch.clamp(kl_mask.sum(dim=1), min=1.0)
            kl_value = (token_kl * kl_mask).sum(dim=1) / kl_denom
        ...
        for key, values in component_lists.items():
            if key == "reward":
                continue
            metrics[f"avg_{key}"] = float(sum(values) / max(len(values), 1))
        return metrics
    ...
    def train(
        ...
        try:
            ...
            total_steps = math.ceil(num_batches / accum_steps) * self.config.num_epochs
        except TypeError:
            total_steps = None
            num_batches = None
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for batch_idx, batch in enumerate(train_dataloader):
                ...
                is_epoch_tail = num_batches is not None and (batch_idx + 1) == num_batches
                    ...
                    if val_dataloader is not None and global_step % self.config.eval_every == 0:
                        ...
                        if val_metrics["reward_rate"] > best_reward_rate:
                            ...
                            if accum_metrics is not None and accum_batches:
                                ...
                                best_metrics["val_reward_rate"] = val_metrics["reward_rate"]
                ...
                if did_step and global_step % self.config.log_every == 0:
                    ...
                    for key, value in sorted(avg_metrics.items()):
                        if key in {
                            ...
                            continue
                        extra_metric_fragments.append(f"{key} {value:.3f}")
                    extra_fragment = ""
                    if extra_metric_fragments:
                        extra_fragment = " " + " ".join(extra_metric_fragments)
                    print(
                        "[grpo] "
                        f"step {step_label} "
                ...
                if did_step:
                    ...
                    wandb_payload["train/learning_rate"] = float(learning_rate)
                    wandb_payload["train/epoch"] = float(epoch + 1)
                    if step_time is not None:
                        wandb_payload["train/step_time_s"] = float(step_time)
                    if val_metrics is not None:
                        for key, value in val_metrics.items():
                            wandb_payload[f"val/{key}"] = float(value)
                    self._log_wandb(wandb_payload, step=global_step)
        ...
        if training_log:
            final_metrics = training_log[-1]["metrics"]
            if self.wandb_run is not None:
                self.wandb_run.summary["final_checkpoint_path"] = final_checkpoint_path
                self.wandb_run.summary["global_step"] = global_step
    ...
    def evaluate(self, val_dataloader) -> Dict[str, float]:
        ...
        for batch in val_dataloader:
            ...
            for idx, prompt in enumerate(prompts):
                ...
                reward = self._score_candidate(text, ground_truth[idx])["reward"]
    ...
    def generate_candidates(
        ...
        with torch.no_grad():
            while input_ids.shape[1] < max_gen_length:
                ...
                last_token_indices = torch.clamp(
                    attention_mask.sum(dim=1).long() - 1,
                    min=0,
                )
                next_token_logits = logits.gather(
                    1,
                    last_token_indices.view(-1, 1, 1).expand(-1, 1, logits.shape[-1]),
                ).squeeze(1)
